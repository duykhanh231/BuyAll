...
public class ProductDetail extends AppCompatActivity {
    ...
    private List<Product_Detail> MyProduct_Detail;
    private ProductDetailCommentAdapter productDetailCommentAdapter;
    private ImageButton ibArrow;
    private ImageView ivIMAGE;
    private TextView tvPRODUCTNAME,tvPRODUCTPRICE,tvPRODUCTSHIPPINGMETHODANDLOCATION,tvRATE,tvPRODUCTDESCRIPTION,tvXEMTATCA;
    private RatingBar rbAVERAGECOMMENT;
    private String SHIPPINGMETHODANDLOCATION,RATE;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rcvComment.setAdapter(productDetailCommentAdapter);
        ...
        tvXEMTATCA.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent = new Intent(ProductDetail.this, FullCommentActivity.class);
                intent.putExtra("PRODUCT_ID", productId); // Gửi productId vào FullCommentActivity
        ...
        for (Product_Detail product_detail : MyProduct_Detail) {
            if (product_detail.getProductId().equals(productId)) {
                tvPRODUCTNAME.setText(product_detail.getProductName());
                String formattedPrice = String.format("đ %,.0f", product_detail.getPrice());
                tvPRODUCTPRICE.setText(formattedPrice);
                SHIPPINGMETHODANDLOCATION = String.format("%s %s | %s %s",
                        "\uD83D\uDE97", product_detail.getShippingMethod(),
                        "\uD83D\uDEA9", product_detail.getLocation());
                tvPRODUCTSHIPPINGMETHODANDLOCATION.setText(SHIPPINGMETHODANDLOCATION);
                rbAVERAGECOMMENT.setRating( product_detail.getAverageRating());
                RATE = String.format("%.1f/5", product_detail.getAverageRating());
                tvRATE.setText(RATE);
                ivIMAGE.setImageResource(product_detail.getImageUrl());
                tvPRODUCTDESCRIPTION.setText(product_detail.getProductDescription());
                break;
